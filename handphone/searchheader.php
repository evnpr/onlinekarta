...
echo 'models["APPLE"] = new Array("',join($rows4, '", "'), '");';
echo 'models["SONY_ERICSSON"] = new Array("',join($rows5, '", "'), '");';
echo 'models["HTC"] = new Array("',join($rows6, '", "'), '");';
...
function resetForm(theForm) {
  /* reset makes */
  theForm.makes.options[0] = new Option("Pilih Merk >>", "Pilih");
  for (var i=0; i<makes.length; i++) {
    theForm.makes.options[i+1] = new Option(makes[i], makes[i]);
  }
  theForm.makes.options[0].selected = true;
  /* reset models */
  theForm.models.options[0] = new Option("Pilih Type >>", "Pilih");
